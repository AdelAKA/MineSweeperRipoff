...
{
    public class StartScreenTab : Tab
    {
        ...
        private void Start()
        {
            ...
            PlayerData.OnGameModeChanged += UpdateSocres;

            easyDifficultyButton.onClick.AddListener(() => GameplayManager.Instance.RequestGame(DifficultyLevel.Easy));
            mediumDifficultyButton.onClick.AddListener(() => GameplayManager.Instance.RequestGame(DifficultyLevel.Medium));
            hardDifficultyButton.onClick.AddListener(() => GameplayManager.Instance.RequestGame(DifficultyLevel.Hard));
        }
        ...
        {
            gameModeText.text = GameplayManager.Instance.CurrentGameMode.ToString();

            float easyScore = PlayerData.GetBestScore(DifficultyLevel.Easy, GameplayManager.Instance.CurrentGameMode);
            if (easyScore == 0)
            ...
            float mediumScore = PlayerData.GetBestScore(DifficultyLevel.Medium, GameplayManager.Instance.CurrentGameMode);
            if (mediumScore == 0)
            ...
            float hardScore = PlayerData.GetBestScore(DifficultyLevel.Hard, GameplayManager.Instance.CurrentGameMode);
            if (hardScore == 0)
